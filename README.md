...
По существующему html файлу, выделяются списки тегов, из которых выделяется информация, включающая: наименование тега; классы описывающие тег (может быть несколько); id, описывающее тег.

> 3 блок:

Производится выборка css правил из css документа. Строится иерархия классов и идентификаторов. Создается новый минимизированный файл min_css(имя).css.

> 4 блок:

Создается минимизированный файл min_html(имя).html.

> 5 блок:

Заканчивается процесс минимизации подгружаемых каскадных таблиц.
