...
Регулярные выражения, используемые для проверки:
1) на вхождение хотя бы одного используемого тега
reg_exp = '([^\w\.]|^)' + cur_tag + r'($|:{1,2}.+$| *\,.+$| ?\..+$|\[.+$| *\~.+$| *>.+$| *\+.+$| +.+$)':
2) на вхождение хотя бы одного используемого класса
reg_exp = '\.' + cur_class + r'($|:{1,2}.+$| *\,.+$| *\..+$|\[.+$| *\~.+$| *>.+$| *\+.+$)'
3) на вхождение хотя бы одного используемого идентификатора
